...
class SyncActionHandler:
    ...
    def force_push(self) -> None:
        """Force push the current branch to its remote.
        
        This method handles force pushing when the user presses 'shift+P'. It
        shows a confirmation dialog first since force pushing can overwrite
        remote history. If the branch doesn't have upstream configured, it
        prompts for that first before showing the confirmation.
        """
        current_branch = self.app._get_current_branch_name()
        if not current_branch:
            self.app.notify("No branch checked out", severity="warning")
        ...
        if not has_upstream or not remote or not remote_branch:
            # No upstream: prompt for upstream first, then force push
            suggested_remote = self.app.sync_service.get_suggested_remote()
            initial_value = f"{suggested_remote} {current_branch}"
            ...
            def on_dialog_result(upstream: str | None) -> None:
                """Handle upstream dialog result, then show force push confirmation."""
                if not upstream:
                    return
                
                try:
                    remote, remote_branch = self.app.sync_service.parse_upstream(upstream)
                    # Show force push confirmation
                    self._confirm_and_force_push(current_branch, remote, remote_branch, set_upstream=True)
                except ValueError as e:
                    message = f"Invalid upstream format: {str(e)}"
                    self.app.notify(message, severity="error", timeout=3.0)
                    self.app.command_log_pane.update_log(message)
            ...
            # Has upstream: show force push confirmation
            self._confirm_and_force_push(current_branch, remote, remote_branch, set_upstream=False)

    def _confirm_and_force_push(
        self, branch_name: str, remote: str, remote_branch: str, set_upstream: bool = False
    ) -> None:
        """Show confirmation dialog and then force push.
        
        Args:
            branch_name: Local branch name.
            remote: Remote name.
            remote_branch: Remote branch name.
            set_upstream: Whether to set upstream during push.
        """
        from ..ui.dialogs import ConfirmDialog
        
        # Create force push prompt message
        prompt = f"Force push {branch_name} to {remote}/{remote_branch}?\n\nThis will overwrite the remote branch."
        ...
        def on_confirm(confirmed: bool) -> None:
            ...
            if result["success"]:
                message = f"Force pushed {branch_name} to {remote}/{remote_branch}"
                self.app.notify(message, severity="success", timeout=2.0)
                self.app.command_log_pane.update_log(message)
                self.app.refresh_data_fast()
            else:
                error_msg = result.get("error", "Unknown error")
                message = f"Failed to force push: {error_msg}"
                self.app.notify(message, severity="error", timeout=5.0)
                self.app.command_log_pane.update_log(message)
